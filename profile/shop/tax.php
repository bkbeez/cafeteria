...
<div class="modal-dialog modal-dialog-centered">
    <div class="modal-content modal-manage">
        <form name="RecordForm" action="<?=$form?>/tax_saving.php" method="POST" enctype="multipart/form-data" class="form-manage" target="_blank">
            <input type="hidden" name="id" value="<?=((isset($data['id'])&&$data['id'])?$data['id']:null)?>"/>
            <div class="modal-header">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <h2 class="mb-0 text-primary text-start on-text-oneline"><i class="uil uil-edit" style="float:left;font-size:36px;line-height:36px;margin-right:3px;"></i><?=Lang::get('TaxNumber')?></h2>
            </div>
            <div class="modal-body">
                <div class="alert alert-primary alert-icon mb-2">
                    <div class="form-floating mb-1">
                        ...
                        <label for="tax_number"><?=Lang::get('Number')?> <span class="text-red">*</span></label>
                    </div>
                </div>
            </div>
            ...
            success: function(rs) {
                ...
                if(data.status=='success'){
                    ...
                    $("form[name='RecordForm'] .modal-footer").fadeOut(1000, function(){
                        $(".card-profile .set-shop-tax").html(data.display);

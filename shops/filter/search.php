...
<?php
    ...
    if( $keyword ){
        ...
        $condition .= " AND ( shop.shop_name LIKE :keyword";
            $condition .= " OR TRIM(CONCAT(COALESCE(shop.title,''),shop.name,' ',COALESCE(shop.surname,''))) LIKE :keyword";
            $condition .= " OR shop.tax_number LIKE :keynum";
            $condition .= " OR shop.phone LIKE :keynum";
        $condition .= " )";
    }
            ...
            if($value){
                if($key=="type"){
                    ...
                    if($value!='ALL'){
                        ...
                        $condition .= " AND shop.type_id=:type";
                    }
                }else if($key=="status"){
                    $_SESSION['login']['filter'][$filter_as]['condition'][$key] = $value;
                    if($value=='ST1'){
                        $condition .= " AND shop.status_id=1";
                    }else if($value=='ST2'){
                        $condition .= " AND shop.status_id=2";
                    }
                }else{
                    $_SESSION['login']['filter'][$filter_as]['condition'][$key] = $value;
    ...
    $count = Shop::one($sql.$condition, $parameters);
    ...
    $sql = "SELECT shop.*
            ...
            WHERE shop.id IS NOT NULL";
    $sql .= $condition;
    $sql .= " ORDER BY shop.id";
    $sql .= " LIMIT $start, $limit;";
    $htmls = '';
    $lists = Shop::sql($sql, $parameters);
        ...
        foreach($lists as $no => $row){
            ...
            $htmls .= '<tr class="'.$row['status'].'">';
                ...
                $htmls .= '<td class="owner">';
                    ...
                    $htmls .= '<span class="fs-sm"><i class="uil uil-phone-volume"></i> '.$row['phone'].'</span>';
                $htmls .= '</td>';

...
<div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content modal-manage">
        <form name="RecordForm" action="<?=$form?>/scripts/update_address.php" method="POST" enctype="multipart/form-data" class="form-manage" target="_blank">
            <input type="hidden" name="id" value="<?=((isset($data['id'])&&$data['id'])?$data['id']:null)?>"/>
            <div class="modal-header">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                <h2 class="mb-0 text-primary text-start on-text-oneline"><i class="uil uil-edit" style="float:left;font-size:36px;line-height:36px;margin-right:3px;"></i><?=Lang::get('AddressBilling')?></h2>
            </div>
            <div class="modal-body">
                <div class="alert alert-primary alert-icon mb-2">
                    <div class="form-floating mb-1">
                        <input value="<?=((isset($data['type_id'])&&$data['type_id'])?$data['type_'.App::lang()]:null);?>" type="text" class="form-control" placeholder="..." disabled>
                        <label><?=Lang::get('Type')?></label>
                    </div>
                    <div class="form-floating mb-1">
                        <input value="<?=((isset($data['shop_name'])&&$data['shop_name'])?$data['shop_name']:null)?>" type="text" class="form-control" placeholder="..." disabled>
                        <label><?=Lang::get('ShopName')?> <span class="text-red">*</span></label>
                    </div>
                    <div class="form-floating mb-1">
                        <input id="tax_number" name="tax_number" value="<?=((isset($data['tax_number'])&&$data['tax_number'])?$data['tax_number']:null)?>" type="text" class="form-control" placeholder="...">
                        <label for="tax_number"><?=Lang::get('TaxNumber')?> <span class="text-red">*</span></label>
                    </div>
                </div>
                <div class="alert alert-primary alert-icon mb-2">
                    <p class="lead text-dark mb-1 text-start on-text-oneline"><?=Lang::get('AddressBilling')?></p>
                    <div class="form-floating mb-1">
                        <input id="address" name="address" value="<?=((isset($data['address'])&&$data['address'])?$data['address']:null)?>" type="text" class="form-control" placeholder="...">
                        <label for="address"><?=Lang::get('Address')?> <span class="text-red">*</span></label>
                    </div>
                    <div class="row gx-1">
                        <div class="col-xs-4 col-sm-8 col-md-8 col-lg-8">
                            <div class="form-floating form-select-wrapper mb-1">
                                <select id="province" name="province" class="form-select"><?=Util::provinceOption(((isset($data['province'])&&$data['province'])?$data['province']:null));?></select>
                                <label for="province"><?=Lang::get('Province')?> <span class="text-red">*</span></label>
                            </div>
                        </div>
                        <div class="col-xs-8 col-sm-4 col-md-4 col-lg-4">
                            <div class="form-floating form-select-wrapper mb-1">
                                <input id="zipcode" name="zipcode" value="<?=((isset($data['zipcode'])&&$data['zipcode'])?$data['zipcode']:null)?>" type="text" class="form-control" placeholder="<?=Lang::get('ZipCode')?>..."/>
                                <label for="zipcode"><?=Lang::get('ZipCode')?></label>
                            </div>
                        </div>
                    </div>
                </div>

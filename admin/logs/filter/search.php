        ...
        foreach($lists as $no => $row){
            $htmls .= '<tr class="'.$row['status'].'">';
                ...
                $htmls .= '<td class="actions">';
                    $htmls .= '<div class="btn-box"><button onclick="manage_events(\'detail\', { \'date_at\':\''.$row['date_at'].'\', \'email\':\''.$row['email'].'\' });" type="button" class="btn btn-sm btn-circle btn-outline-primary"><i class="uil uil-info"></i></button><small class=b-tip>'.$lang_data.'</small></div>';
                $htmls .= '</td>';
            $htmls .= '</tr>';
        }
    }

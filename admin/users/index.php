                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-8 col-sm-8 col-md-8 col-lg-6 filter-keyword">
                            <div class="mc-field-group input-group form-floating mb-1">
                                ...
                                <button type="button" class="btn btn-primary btn-adding" title="Create New" onclick="manage_events('new', { 'link':'<?=$link?>' });"><i class="uil uil-plus"></i><span> <?=( (App::lang()=='en') ? 'New User' : 'ผู้ใช้ใหม่' )?></span></button>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                            <div class="form-floating form-select-wrapper mb-1">
                                <select name="condition[role]" class="form-select">
                                    <option value="ALL"<?=((!isset($filter['condition']['role'])||$filter['condition']['role']=='ALL')?' selected':null)?>><?=Lang::get('All')?></option>
                                    <option value="ADMIN"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='ADMIN')?' selected':null)?>>[ADMIN] <?=( (App::lang()=='en') ? 'Administrator' : 'ผู้ดูแลระบบ' )?></option>
                                    <option value="STAFF"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='STAFF')?' selected':null)?>>[STAFF] <?=( (App::lang()=='en') ? 'Officer' : 'เจ้าหน้าที่' )?></option>
                                    <option value="USER"<?=((isset($filter['condition']['role'])&&$filter['condition']['role']=='USER')?' selected':null)?>>[USER] <?=( (App::lang()=='en') ? 'User' : 'ผู้ใช้ทั่วไป' )?></option>
                                </select>
                                <label><?=Lang::get('Account')?></label>
                            </div>
                        </div>
                        <div class="col-xs-12 col-sm-6 col-md-3 col-lg-3">
                            <div class="form-floating form-select-wrapper mb-1">
                                <select name="condition[cmu]" class="form-select">
                                    <option value="ALL"<?=((!isset($filter['condition']['cmu'])||$filter['condition']['cmu']=='ALL')?' selected':null)?>><?=Lang::get('All')?></option>
                                    <option value="CMU"<?=((isset($filter['condition']['cmu'])&&$filter['condition']['cmu']=='CMU')?' selected':null)?>><?=( (App::lang()=='en') ? 'CMU Account' : 'บัญชี CMU' )?></option>
                                    <option value="NOT"<?=((isset($filter['condition']['cmu'])&&$filter['condition']['cmu']=='NOT')?' selected':null)?>><?=( (App::lang()=='en') ? 'Not CMU Account' : 'ไม่ใช่บัญชี CMU' )?></option>
                                </select>
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="type"><?=Lang::get('UserAccount')?></th>
    ...
    function manage_events(action, params){
        if(action=='new'){
            ...
            swal({
                ...
                'html' : '<div class="fs-24 text-red on-font-primary mb-2">'+params.email+'</div><div><?=Lang::get('ConfirmToDelete')?></div>',

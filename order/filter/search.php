        ...
        foreach($lists as $no => $row){
            ...
            $htmls .= '<tr class="'.$row['status'].'">';
                ...
                if( $row['status_id']>=4 ){
                    ...
                    $htmls .= '<div class="btn-box warning"><button onclick="manage_events(\'check\', { \'id\':\''.$row['id'].'\' });" type="button" class="btn btn-sm btn-circle btn-warning"><i class="uil uil-file-edit-alt"></i></button><small class=b-tip>'.$lang_view.'</small></div>';
                }
                $htmls .= '</td>';
            $htmls .= '</tr>';
        }
